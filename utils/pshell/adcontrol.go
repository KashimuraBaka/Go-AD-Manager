...
func (ps *ADControl) EnableUser(user string, unlock bool) (err error) {
	// TODO: OU/OU=子用户组/用户组, DC/DC=二级域名/顶级域名
	if unlock {
		_, _, err = ps.Execute(
			fmt.Sprintf("Enable-ADAccount '%s';", user),
			fmt.Sprintf("Get-ADUser '%s' | Move-ADObject -TargetPath 'OU={OU},OU={OU},DC={DC},DC={DC}';", user),
		)
	} else {
		_, _, err = ps.Execute(
			fmt.Sprintf("Disable-ADAccount '%s';", user),
			fmt.Sprintf("Get-ADUser '%s' | Move-ADObject -TargetPath 'OU=Disable,OU={OU},DC={DC},DC={DC}';", user),
		)
	}
	return
}
...
func (ps *ADControl) GetUsers(user string) ([]ADUser, error) {
	...
	if user == "" {
		...
		$ADUser = Get-ADUser -Filter * -SearchBase 'OU={OU},DC={DC},DC={DC}' -properties logoncount,CanonicalName,lastlogon,badpasswordtime,badPwdCount,pwdLastSet;
		$ADUser = $ADUser | select name,enabled,logoncount,CanonicalName,lastlogon,badpasswordtime,badPwdCount,pwdLastSet;
		$ADComputer = Get-ADComputer -Filter * -SearchBase 'OU={OU},DC={DC},DC={DC}' -properties Name,Description,OperatingSystem,OperatingSystemVersion;
